...
function classificaTriangulo(ladoA, ladoB, ladoC) {
  ...
  if (ladoA != ladoB && ladoA != ladoC && ladoB != ladoC) {
    ...
    (ladoA === ladoC && ladoB != ladoC && ladoB != ladoA) ||
...
function retornaChamadaDeFilme(filme) {
  console.log(filme);
  const imprimir = `Venha assistir ao filme ${filme.nome}, de ${
    filme.ano
...
function retornaArrayOrdenadoAlfabeticamente(consultas) {
  ...
  let ordenados = consultas.sort((a, b) => {
    if (a.nome > b.nome) {
      return 1;
...
function retornaArrayOrdenadoPorData(consultas) {
  for (let i = 0; i < consultas[0].dataDaConsulta.length; i++) {
    consultas.sort((a, b) => {
      return a.dataDaConsulta.split("/")[i] - b.dataDaConsulta.split("/")[i];
    });
  }
